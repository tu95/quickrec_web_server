...
export function validateOssConfig(input) {
  ...
  const legacyPrefix = trimText(oss.objectPrefix)
  const fallbackPrefixBase = legacyPrefix || 'recordings'
  const objectPrefixMp3 = trimText(oss.objectPrefixMp3) || `${fallbackPrefixBase}/mp3`
  const objectPrefixOpus = trimText(oss.objectPrefixOpus) || `${fallbackPrefixBase}/opus`
  const normalized = {
